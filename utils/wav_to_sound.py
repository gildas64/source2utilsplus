...
PATH_TO_CONTENT_ROOT = (len(sys.argv) == 2 and sys.argv[1] or \
    input("What folder would you like to convert? Valid Format: C:\\Steam\\steamapps\\Half-Life Alyx\\content\\tf\\sounds\\props_spytech: ")).lower()
...
for filename in files:
    ...
    with open(out_name, 'w') as out:
        putl(out, SOUND_BASE.replace('<sound>', sound_path.replace(
            INPUT_FILE_EXT, ".vsnd")).replace((' ' * 4), '\t'))
